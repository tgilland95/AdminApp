...
export function updatePendingRecord(row, itemID, dept, func, recType, catID, cat, ret, exc, adminCmts, flag) {
  ...
  $.ajax({
    ...
    failure: function () {
      $('#approve-alert').html('</br><div class="alert alert-warning" role="alert">Server error. Please try agai' +
          'n.</div>')
    }
  })
}
...
export function addRecord(dept, code, recType, recFunc, recCat, userMsg, commentsPlan, archival, vital, highlyConfidential, recRepo) {
  ...
  $.ajax({
    ...
    failure: function () {
      $('#add-alert').html('</br><div class="alert alert-warning" role="alert">Server error. Record not adde' +
          'd.</div>')
      setTimeout(function () {
        $('#add-alert').empty()
      }, 2500)
